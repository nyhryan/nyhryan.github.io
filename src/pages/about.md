---
layout: ../layouts/MarkdownLayout.astro
title: About
description: You can learn about me in this page.
---

# About

## About me

Hi, this is the 'about' page of me. I usually go with the nickname 'ATAI' in online.

### Academic background

- 2022~2026:  
  Computer science barchelor's degree @ Hansung University, Seoul, S.Korea.
  - 2024~2025: Two-year full scholarship from KOSAF(**국가우수장학금(이공계)**). [Details(Korean)](https://www.kosaf.go.kr/ko/scholar.do?pg=scholarship05_06_01&naviParam=JH,04,03,11)
  - Final GPA: _4.46/4.5_
- 2026~:  
  🚧 Building experiences... 🚧

### Skills

- Programming languages I like: `C, C++, Java, JavaScript, Go`
  - Have experience with `Spring boot, Docker, OpenGL`
- Language skills
  - English: ~~TOEIC 945~~ (Expired since 2021)
  - Japanese: JLPT N1 (2022.01)

### Interests

- Gaming 🎮
  - Long time player of Overwatch(Since 2015)
  - Arcade rhythm games, mainly KONAMI Bemani series
    - Beatmania IIDX SP 中伝
    - Currently grinding Pop'n Music LV.48~49
- Music 🎵
  - Huge fan of J-POP, Doujin music
  - I also used to produce music. You can check out songs I made in my [SoundCloud page](https://soundcloud.com/a_i_1002)
  - Also, you can check out my song **Turbocharger** in the mobile rhythm game 'Arcaea'

### Socials

- GitHub: [nyhryan](https://github.com/nyhryan)

## About this blog

This blog is built with the following tools.

- [Astro framework](https://astro.build/)
- [tailwindcss](https://tailwindcss.com/)
- [Alpine.js - Your new, lightweight, JavaScript framework.](https://alpinejs.dev/)
- [swup - Versatile and extensible page transition library for server-rendered websites](https://swup.js.org/)
- [Bun](https://bun.com/)

You can check the project repository [here](https://github.com/nyhryan/nyhryan.github.io/).
