---
layout: post
title: "[C언어로 쉽게 풀어쓴 자료구조] 동적 크기 스택"
...
- c
- data structure
img_path: "/assets/img/posts/data-structure/2023-06-26-c언어로-쉽게-풀어쓴-자료구조-동적-크기-스택/"
---

<!--more-->

> - 이전 포스트 : [[C언어로 쉽게 풀어쓴 자료구조] 스택의 기초와 고정 크기 스택]({% post_url 2023-06-20-C언어로-쉽게-풀어쓴-자료구조-스택의-기초와-고정-크기-스택 %})
> - 다음 포스트 : 
...
```
`realloc()`은 `malloc()` 함수와 마찬가지로 힙 공간에 접근하는 함수이다. 함수 실행 후 `void*` 포인터를 리턴하므로 배열의 자료형에 맞게 캐스팅해줄 필요가 있다. 매개변수로는 이동할 메모리 블럭 (여기서는 스택의 배열), 그리고 새로운 크기 (바이트 단위) 2가지를 필요로 한다.
...
`realloc()`을 통해 크기를 조정해야하기 때문에, 스택 변수인 일반 배열 `arr[]` 대신에 우선 포인터 변수로 선언해두고, 초기화 함수 `stack_init()` 에서 `malloc()`을 통해 배열의 크기만큼 힙에서 할당받는다.

이후 초기화 함수 `stack_init()`에서 올바른 초기화 작업을 해준다. 
